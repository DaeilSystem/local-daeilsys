...
interface DaeilsysProductsSectionProps {
  className?: string
}

export function DaeilsysProductsSection({ className }: DaeilsysProductsSectionProps) {
  const { language, isInitialized } = useLanguage()
  const productData = isInitialized && language === "ko" ? products : productsEn

  const productStories = productData.slice(0, 5).map((product) => ({
    id: product.id,
    title: isInitialized && language === "ko" ? product.nameKo : product.name,
    image: product.image,
    alt: isInitialized && language === "ko" ? product.nameKo : product.name
  }))
        ...
        <div className="text-center space-y-4 sm:space-y-6">
          ...
          <p className="text-base sm:text-lg md:text-xl text-gray-600 dark:text-gray-400 leading-relaxed max-w-4xl mx-auto">
            {isInitialized && language === "ko"
              ? "나노 스케일 측정 장비를 위한 진동 절연 시스템 설계 제조 전문 기업으로서, 35년 이상의 경험과 기술을 바탕으로 최상의 성능의 미진동 제어 시스템을 제공합니다."
              : "As a specialized company in designing and manufacturing vibration isolation systems for nanoscale measurement equipment, we provide the highest performance vibration control systems based on over 35 years of experience and technology."
            }
          </p>
        </div>
      </div>
