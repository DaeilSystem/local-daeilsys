...
import productData from "@/data/products/dvip-c-full.json"
import { useLanguage } from "@/hooks/use-language"
import Link from "next/link"
import { useState } from "react"

type TabType = "overview" | "performance" | "specifications" | "applications" | "resources"

export default function DVIPCClient() {
  ...
  const tabs: { id: TabType; label: { en: string; ko: string } }[] = [
    ...
    { id: "applications", label: { en: "Applications", ko: "설치 사례" } },
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-800">
                    ...
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Details" : "상세"}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {productData.tabs.specifications.rows.map((row, rowIndex) => (
                    <tr
                      key={rowIndex}
        ...
        {activeTab === "applications" && (
          <div className="space-y-8">
            <h2 className="text-3xl font-semibold text-gray-900 dark:text-white">
              {language === "en" ? "Installation Photos" : "설치 사진"}
            </h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {productData.tabs.applications.map((application, index) => (
                <div key={index} className="space-y-4">
                  {application.image && (
                    <div className="rounded-lg overflow-hidden shadow-lg dark:shadow-xl">
                      <img
                        src={application.image}
                        alt={typeof application.title === "string" ? application.title : application.title[language]}
                  ...
                  {application.title && (
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white text-center">
                      {typeof application.title === "string" ? application.title : application.title[language]}
                    </h3>
                  )}
                </div>
              ))}
