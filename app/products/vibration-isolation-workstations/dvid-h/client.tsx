"use client"

import {
  Carousel,
  CarouselContent,
  CarouselItem,
  CarouselNext,
  CarouselPrevious,
} from "@/components/ui/carousel"
import productData from "@/data/products/dvid-h-full.json"
import { useLanguage } from "@/hooks/use-language"
import Link from "next/link"
import { useState } from "react"

type TabType = "overview" | "features" | "performance" | "specifications" | "order" | "options"

export default function DvidHClient() {
  const { language } = useLanguage()
  const [activeTab, setActiveTab] = useState<TabType>("overview")

  const tabs: { id: TabType; label: { en: string; ko: string } }[] = [
    { id: "overview", label: { en: "Overview", ko: "개요" } },
    { id: "features", label: { en: "Features", ko: "특징" } },
    { id: "performance", label: { en: "Performance", ko: "성능" } },
    { id: "specifications", label: { en: "Specifications", ko: "사양" } },
    { id: "order", label: { en: "Order", ko: "주문 정보" } },
    { id: "options", label: { en: "Options", ko: "옵션" } },
  ]

  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      {/* Hero Section */}
      <div className="bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-950 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h1 className="text-5xl font-light text-gray-900 dark:text-white mb-4">
              {typeof productData.fullName === "string"
                ? productData.fullName
                : productData.fullName[language]}
            </h1>
            <h2 className="text-3xl text-blue-600 dark:text-blue-400 mb-4">
              {typeof productData.series === "string"
                ? productData.series
                : productData.series[language]}
            </h2>
            <p className="text-xl text-gray-600 dark:text-gray-300">
              {typeof productData.tagline === "string"
                ? productData.tagline
                : productData.tagline[language]}
            </p>
          </div>

          {/* Hero Image Carousel */}
          <div className="max-w-4xl mx-auto">
            <Carousel className="w-full">
              <CarouselContent>
                {productData.heroImages.map((image, index) => (
                  <CarouselItem key={index}>
                    <div className="aspect-video bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden">
                      <img
                        src={image}
                        alt={`${productData.name} ${index + 1}`}
                        className="w-full h-full object-contain"
                      />
                    </div>
                  </CarouselItem>
                ))}
              </CarouselContent>
              <CarouselPrevious className="left-4" />
              <CarouselNext className="right-4" />
            </Carousel>
          </div>
        </div>
      </div>

      {/* Tabs Navigation */}
      <div className="border-b border-gray-200 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-950 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex space-x-8 overflow-x-auto">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`py-4 px-2 border-b-2 font-medium text-sm transition-colors whitespace-nowrap ${
                  activeTab === tab.id
                    ? "border-blue-600 text-blue-600 dark:text-blue-400"
                    : "border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
                }`}
              >
                {tab.label[language]}
              </button>
            ))}
          </nav>
        </div>
      </div>

      {/* Tab Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        {/* Overview Tab */}
        {activeTab === "overview" && (
          <div className="space-y-8">
            <div className="prose dark:prose-invert max-w-none">
              <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed">
                {typeof productData.tabs.overview.description === "string"
                  ? productData.tabs.overview.description
                  : productData.tabs.overview.description[language]}
              </p>
            </div>
          </div>
        )}

        {/* Features Tab */}
        {activeTab === "features" && (
          <div className="space-y-12">
            {productData.tabs.features.map((feature, index) => (
              <div key={index} className="space-y-4">
                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">
                  {typeof feature.title === "string"
                    ? feature.title
                    : feature.title[language]}
                </h3>
                <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
                  {typeof feature.description === "string"
                    ? feature.description
                    : feature.description[language]}
                </p>
              </div>
            ))}
          </div>
        )}

        {/* Performance Tab */}
        {activeTab === "performance" && (
          <div className="max-w-4xl mx-auto space-y-8">
            <h2 className="text-3xl font-semibold text-gray-900 dark:text-white text-center">
              {typeof productData.tabs.performance.title === "string"
                ? productData.tabs.performance.title
                : productData.tabs.performance.title[language]}
            </h2>
            <p className="text-lg text-gray-600 dark:text-gray-300 text-center">
              {typeof productData.tabs.performance.description === "string"
                ? productData.tabs.performance.description
                : productData.tabs.performance.description[language]}
            </p>
            {productData.tabs.performance.image && (
              <div className="rounded-lg overflow-hidden">
                <img
                  src={productData.tabs.performance.image}
                  alt="DVID-H Performance"
                  className="w-full h-auto object-contain"
                />
              </div>
            )}
          </div>
        )}

        {/* Specifications Tab */}
        {activeTab === "specifications" && (
          <div className="space-y-12">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-800">
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Construction" : "구조"}
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Performance" : "성능"}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Core" : "코어"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.core === "string"
                        ? productData.tabs.specifications.construction.core
                        : productData.tabs.specifications.construction.core[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Isolation System" : "제진 시스템"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.isolationSystem === "string"
                        ? productData.tabs.specifications.performance.isolationSystem
                        : productData.tabs.specifications.performance.isolationSystem[language]}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Surface Flatness" : "표면 평탄도"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.surfaceFlatness === "string"
                        ? productData.tabs.specifications.construction.surfaceFlatness
                        : productData.tabs.specifications.construction.surfaceFlatness[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Isolator Type" : "아이솔레이터 타입"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.isolatorType === "string"
                        ? productData.tabs.specifications.performance.isolatorType
                        : productData.tabs.specifications.performance.isolatorType[language]}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Top Skin" : "상판 스킨"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.topSkin === "string"
                        ? productData.tabs.specifications.construction.topSkin
                        : productData.tabs.specifications.construction.topSkin[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Resonant Frequency" : "공진 주파수"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.resonantFrequency === "string"
                        ? productData.tabs.specifications.performance.resonantFrequency
                        : productData.tabs.specifications.performance.resonantFrequency[language]}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Bottom Skin" : "하판 스킨"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.bottomSkin === "string"
                        ? productData.tabs.specifications.construction.bottomSkin
                        : productData.tabs.specifications.construction.bottomSkin[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Vibration Isolation at 10Hz" : "10Hz 제진 성능"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.vibrationIsolationAt10Hz === "string"
                        ? productData.tabs.specifications.performance.vibrationIsolationAt10Hz
                        : productData.tabs.specifications.performance.vibrationIsolationAt10Hz[language]}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Mounting Holes" : "마운팅 홀"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.mountingHoles === "string"
                        ? productData.tabs.specifications.construction.mountingHoles
                        : productData.tabs.specifications.construction.mountingHoles[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Maximum Load Capacity" : "최대 하중"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.maxLoadCapacity === "string"
                        ? productData.tabs.specifications.performance.maxLoadCapacity
                        : productData.tabs.specifications.performance.maxLoadCapacity[language]}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Mounting Hole Patterns" : "마운팅 홀 패턴"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.mountingHolePatterns === "string"
                        ? productData.tabs.specifications.construction.mountingHolePatterns
                        : productData.tabs.specifications.construction.mountingHolePatterns[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Automatic Leveling" : "자동 수평 조절"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.automaticLeveling === "string"
                        ? productData.tabs.specifications.performance.automaticLeveling
                        : productData.tabs.specifications.performance.automaticLeveling[language]}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Mounting Hole Borders" : "마운팅 홀 경계"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.mountingHoleBorders === "string"
                        ? productData.tabs.specifications.construction.mountingHoleBorders
                        : productData.tabs.specifications.construction.mountingHoleBorders[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Leveling Repeatability" : "수평 조절 반복성"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.levelingRepeatability === "string"
                        ? productData.tabs.specifications.performance.levelingRepeatability
                        : productData.tabs.specifications.performance.levelingRepeatability[language]}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Mounting Hole Sealing" : "마운팅 홀 씰링"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.mountingHoleSealing === "string"
                        ? productData.tabs.specifications.construction.mountingHoleSealing
                        : productData.tabs.specifications.construction.mountingHoleSealing[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Required Air Supply" : "필요 공기 공급"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.requiredAirSupply === "string"
                        ? productData.tabs.specifications.performance.requiredAirSupply
                        : productData.tabs.specifications.performance.requiredAirSupply[language]}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Damping" : "댐핑"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.construction.damping === "string"
                        ? productData.tabs.specifications.construction.damping
                        : productData.tabs.specifications.construction.damping[language]}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Height Adjustment" : "높이 조절"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                      {typeof productData.tabs.specifications.performance.heightAdjustment === "string"
                        ? productData.tabs.specifications.performance.heightAdjustment
                        : productData.tabs.specifications.performance.heightAdjustment[language]}
                    </td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        )}

        {/* Order Tab */}
        {activeTab === "order" && (
          <div className="space-y-12">
            {productData.tabs.order.image && (
              <div className="mb-8 rounded-lg overflow-hidden">
                <img
                  src={productData.tabs.order.image}
                  alt="DVID-H Ordering Information"
                  className="w-full h-auto"
                />
              </div>
            )}
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-800">
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Model No." : "모델 번호"}
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Vibration Isolated Area" : "제진 영역"}
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Dimensions" : "치수"}
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700">
                      {language === "en" ? "Weight" : "무게"}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {productData.tabs.order.models.map((model, index) => (
                    <tr key={index}>
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                        {model.modelNo}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                        {model.isolatedArea}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                        {model.dimensions}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                        {model.weight} kg
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400 italic mt-4">
              {language === "en"
                ? "* Custom dimensions and configurations are available upon request."
                : "* 맞춤형 치수 및 구성은 요청 시 제공됩니다."}
            </p>
          </div>
        )}

        {/* Options Tab */}
        {activeTab === "options" && (
          <div className="space-y-12">
            <h2 className="text-3xl font-semibold text-gray-900 dark:text-white text-center">
              {language === "en" ? "Available Options" : "사용 가능한 옵션"}
            </h2>
            <div className="grid md:grid-cols-3 gap-8">
              {productData.tabs.options.map((option, index) => (
                <div key={index} className="space-y-4 text-center">
                  <div className="rounded-lg overflow-hidden shadow-lg dark:shadow-xl">
                    <img
                      src={option.image}
                      alt={typeof option.title === "string" ? option.title : option.title[language]}
                      className="w-full h-64 object-cover"
                    />
                  </div>
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                    {typeof option.title === "string" ? option.title : option.title[language]}
                  </h3>
                  <p className="text-gray-600 dark:text-gray-300">
                    {typeof option.description === "string" ? option.description : option.description[language]}
                  </p>
                </div>
              ))}
            </div>
          </div>
        )}
      </div>

      {/* CTA Section */}
      <div className="bg-gray-50 dark:bg-gray-900 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-semibold text-gray-900 dark:text-white mb-8">
            Ready to get started?
          </h2>
          <Link
            href="/contact"
            className="inline-block bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 transition-colors text-lg"
          >
            Contact us
          </Link>
        </div>
      </div>
    </div>
  )
}
