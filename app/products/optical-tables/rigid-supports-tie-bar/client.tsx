...
import productData from "@/data/products/rigid-supports-tie-bar-full.json"
import { useLanguage } from "@/hooks/use-language"
import Link from "next/link"
import { useState } from "react"

type TabType = "overview" | "options" | "order" | "resources"

export default function RigidSupportsTieBarClient() {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        {activeTab === "options" && (
          <div className="space-y-12">
            ...
            {/* Related Products */}
            {productData.tabs.relatedProducts.length > 0 && (
              <div>
                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mb-6">
                  {language === "en" ? "Related Products" : "관련 제품"}
                </h3>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {productData.tabs.relatedProducts.map((product, index) => (
                    <Link
                      key={index}
                      href={product.url}
                    ...
                    >
                      {product.image && (
                        <div className="rounded-lg overflow-hidden shadow-lg dark:shadow-xl group-hover:shadow-xl transition-shadow">
                          <img
                            src={product.image}
                            alt={typeof product.title === "string" ? product.title : product.title[language]}
                            className="w-full h-48 object-contain bg-gray-50 dark:bg-gray-800"
                          />
                        </div>
                      )}
                      <h4 className="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                        {typeof product.title === "string" ? product.title : product.title[language]}
                      </h4>
                    </Link>
                  ))}
                </div>
        ...
        {activeTab === "order" && (
          <div className="space-y-8">
            ...
            {/* 4 Post Image */}
            {productData.tabs.order.post4Image && (
              <div className="max-w-2xl mx-auto mb-8">
                ...
                <div className="rounded-lg overflow-hidden shadow-lg dark:shadow-xl">
                  <img
                    src={productData.tabs.order.post4Image}
                    alt="4 Post with Tie-Bar"
                    className="w-full h-auto object-contain"
                  />
                </div>
              </div>
            ...
            {/* 6 Post Image */}
            {productData.tabs.order.post6Image && (
              <div className="max-w-2xl mx-auto mb-8">
                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4 text-center">
                  6 Post with Tie-Bar
                </h3>
                <div className="rounded-lg overflow-hidden shadow-lg dark:shadow-xl">
                  <img
                    src={productData.tabs.order.post6Image}
                    alt="6 Post with Tie-Bar"
                    className="w-full h-auto object-contain"
                  />
                </div>
              </div>
            ...
            {/* Order Note */}
            {productData.tabs.order.note && (
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center italic mb-6">
                {typeof productData.tabs.order.note === "string"
                  ? productData.tabs.order.note
                  : productData.tabs.order.note[language]}
              </p>
            )}
