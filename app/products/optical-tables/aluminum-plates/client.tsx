...
import productData from "@/data/products/aluminum-plates-full.json"
import { useLanguage } from "@/hooks/use-language"
import Link from "next/link"
import { useState } from "react"

type TabType = "overview" | "specifications" | "order" | "resources"

export default function AluminumPlatesClient() {
                  ...
                  {productData.tabs.order.rows.slice(1).map((row, rowIndex) => (
                    ...
                    >
                      {row.map((cell, cellIndex) => (
                        <td
                          key={cellIndex}
                          className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300 whitespace-pre-line"
                        >
                          {cell}
                        </td>
                      ))}
