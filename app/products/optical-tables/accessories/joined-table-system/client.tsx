...
import productData from "@/data/products/joined-table-system-full.json"
import { useLanguage } from "@/hooks/use-language"
import Link from "next/link"
import { useState } from "react"

type TabType = "overview" | "resources"

export default function JoinedTableSystemClient() {
          ...
          <div className="space-y-12">
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {productData.tabs.overview.features.map((feature, index) => (
            ...
            {/* Layout Examples */}
            {productData.tabs.layoutExamples.length > 0 && (
              <div>
                <h2 className="text-3xl font-semibold text-gray-900 dark:text-white mb-8">
                  {language === "en" ? "Joined Optical Table Layout Examples" : "조인드 광학 테이블 레이아웃 예시"}
                </h2>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {productData.tabs.layoutExamples.map((example, index) => (
                    <div key={index} className="space-y-4">
                      <div className="rounded-lg overflow-hidden shadow-lg dark:shadow-xl">
                        <img
                          src={example.image}
                          alt={typeof example.title === "string" ? example.title : example.title[language]}
                      ...
                      <h4 className="text-lg font-semibold text-gray-900 dark:text-white text-center">
                        {typeof example.title === "string" ? example.title : example.title[language]}
                      </h4>
                      {example.description && (
